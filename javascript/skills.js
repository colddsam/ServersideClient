...
function postData() {
        ...
        obj.innerHTML=`<form class="swing-in-top-fwd" id="uploadSkills" enctype="multipart/form-data">
        <label for="name">Name:</label>
        <input class="shadow-pop-br" type="text" placeholder="name" name="name" required>
        <br>
        <label for="percentage">Percentage:</label>
        <input class="shadow-pop-br" type="number" placeholder="percentage" name="percentage" required>
        <br>
        ...
        <br>
        <button class="shake-bottom" type="button" onclick="uploadSkills()">Upload</button>
    </form>`
}
...
function uploadSkills() {
            const form = document.getElementById('uploadSkills');
    ...
    if (flag[0] == 0) {
        ...
        fetch('https://portfoliobackend-hnx5.onrender.com/skills/', {
                method: 'POST',
                body: formData,
...
async function onDelete(idx) {
    ...
    try {
        const response = await fetch(`https://portfoliobackend-hnx5.onrender.com/delete/skills/${idx}`);
