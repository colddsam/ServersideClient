...
function postData() {
        ...
        obj.innerHTML=`<form class="swing-in-top-fwd" id="uploadAchievements" enctype="multipart/form-data">
        <label for="header">Header:</label>
        <input class="shadow-pop-br" type="text" placeholder="header" name="header" required>
        <br>
        ...
        <br>
        <button class="shake-bottom" type="button" onclick="uploadAchievements()">Upload</button>
    </form>`
}
...
function uploadAchievements() {
            const form = document.getElementById('uploadAchievements');
    ...
    if (flag[0] == 0) {
        ...
        fetch('https://portfoliobackend-hnx5.onrender.com/achievements/', {
                method: 'POST',
                body: formData,
...
async function onDelete(idx) {
    ...
    try {
        const response = await fetch(`https://portfoliobackend-hnx5.onrender.com/delete/achievements/${idx}`);
