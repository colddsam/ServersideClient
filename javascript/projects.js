...
function postData() {
        ...
        obj.innerHTML=`<form class="swing-in-top-fwd" id="uploadProjects" enctype="multipart/form-data">
        ...
        <br>
        <button class="shake-bottom" type="button" onclick="uploadProjects()">Upload</button>
    </form>`
}
...
function uploadProjects() {
            const form = document.getElementById('uploadProjects');
    ...
    if (flag[0] == 0) {
        ...
        fetch('https://portfoliobackend-hnx5.onrender.com/projects/', {
                method: 'POST',
                body: formData,
...
async function onDelete(idx) {
    ...
    try {
        const response = await fetch(`https://portfoliobackend-hnx5.onrender.com/delete/projects/${idx}`);
